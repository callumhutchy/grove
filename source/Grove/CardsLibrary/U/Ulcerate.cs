  ...
  public class Ulcerate : CardTemplateSource
  {
    public override IEnumerable<CardTemplate> GetCards()
    {
      yield return Card
        ...
        .FlavorText(
          "\"If it were merely lethal, that would be sufficient. The art, however, is in maximizing the suffering it causes.\"—Liliana Vess")
        .Cast(p =>
          {
            p.Effect = () => new CompoundEffect(
              new ApplyModifiersToTargets(
                () => new AddPowerAndToughness(-3, -3) {UntilEot = true}) {ToughnessReduction = 3},
              new ChangeLife(amount: -3, whos: P(e => e.Controller)));
